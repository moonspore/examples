...
3. Launched an Android emulator using `sudo ./emulator -avd Pixel_3a_API_30_x86` to create a _Pixel 3a_ device
4. Installed the React Native app on the emulator using `npx react-native run-android`
5. Launched a second emulator using `sudo ./emulator -avd Pixel_3a_API_30_x86 -read-only` to create a second _Pixel 3a_ device (since the same command was used to launch the emulator, the React Native app is already installed on the second emulator when it launches)
...
The first device initialized the agent using the wallet label _My Wallet Alice_ and the second device initialized the agent using the wallet label _My Wallet Bob_

The _My Wallet Alice_ agent will be now be referred to as _Alice's agent_ and the _My Wallet Bob_ agent will now be referred to as _Bob's agent_.
...
agent.events.on(BasicMessageEventTypes.BasicMessageStateChanged, (e) => {
  if (e.payload.basicMessageRecord.role === "receiver") {
    ...
    agent.basicMessages.sendMessage(
      e.payload.basicMessageRecord.connectionId,
      "message received"
    );
  }
});
...
Both Alice and Bob can successfully exchange basic messages _with the mediator_.
...
Both Alice's agent and Bob's agent can successfully exchange basic messages _with each other_.
...
httpInboundTransport.app.get("/offerlist", async (req, res) => {
  ...
  conns.forEach((conn, i) => {
    ...
    list =
      list +
      `<a href="http://192.168.1.72:3001/offer?connection=` +
      conn.id +
      `&name=` +
      shortName +
      `">` +
...
8. The proper credential and requested attributes have been retrieved and next the request will be accepted and a response will be sent to Bob's agent by this code:
...
9. In the console on Bob's agent the following requested proof is displayed:
...
agent.events.on <
  ...
  async (event) => {
    ...
    if (payload.proofRecord.state === "presentation-received") {
      var attached =
        payload.proofRecord.presentationMessage.presentationAttachments[0].data
          .base64;
      ...
      if (payload.proofRecord.isVerified) {
        var vc = {
          ...
          phone: sp.requested_proof.revealed_attrs.phone.raw,
        };
